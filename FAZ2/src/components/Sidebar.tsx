...
import {
  ...
  BarChart3, User, Baby, ChevronRight
} from 'lucide-react';
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13.5px] font-semibold transition-all duration-200 ${
                isActive
                  ? `${accent.activeBg} ${accent.text} border-l-[3px] ${accent.border}`
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2 rounded-xl bg-white/80 backdrop-blur-lg shadow-lg border border-gray-200/50"
      >
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black/30 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="w-[280px] h-full bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
