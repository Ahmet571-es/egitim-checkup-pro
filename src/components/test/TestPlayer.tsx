...
// ============================================================
// TestPlayer — Ortak psikometrik test motoru
// ------------------------------------------------------------
// Hem öğrenci (/student/my-tests) hem öğretmen (/teacher/my-tests)
// panellerinde kullanılır. Test akışı, skorlama ve arayüz aynıdır;
// SADECE sonucun nereye kaydedileceği (saver) ve kayıt sonrası yan
// etki (onSaved — ör. öğrenci gamification) prop olarak enjekte edilir.
// ============================================================
...
export interface TestPlayerProps {
  testId: string;
  /** Sonucu ilgili tabloya yazar. Hata olursa { error } döner. */
  saver: (payload: TestSavePayload) => Promise<{ error?: { code?: string; message?: string } | null }>;
  /** Kayıt başarılı olduğunda çağrılır (opsiyonel; ör. öğrenci XP/rozet). */
  onSaved?: (info: { takerId: string; testId: string; mainScore: number }) => void | Promise<void>;
  /** "Test bulunamadı" ekranındaki geri dönüş linki. */
  backHref?: string;
}
...
export default function TestPlayer({ testId, saver, onSaved, backHref = '/student/my-tests' }: TestPlayerProps) {
  ...
  const doSaveToDb = useCallback(async () => {
    ...
    try {
      ...
      // studentId burada "mevcut kimlikli kullanıcının id'si" demektir
      // (öğrenci veya öğretmen — TestPlayer her ikisinde de kullanılır).
      let currentTakerId = studentId;
      if (!currentTakerId) {
        const { data: { user } } = await supabase.auth.getUser();
        if (user?.id) {
          currentTakerId = user.id;
      ...
      //   öğretmen → teacher_test_results (izole, öğretmene özel)
      ...
      if (error) {
        console.error('[DB save] sonuç kaydı hatası:', error);
        setSaveStatus('error');
        setSaveError(friendlyError(error));
      } else {
        console.log('[DB save] sonuç kaydedildi:', testId);
        setDbSaved(true);
        setSaveStatus('saved');

        // Kayıt sonrası yan etki (opsiyonel — ör. öğrenci XP/rozet).
        // Öğretmen akışında onSaved geçilmez → gamification tetiklenmez.
        try {
          const mainScore = typeof scoresObj._main === 'number' ? scoresObj._main : 50;
          await onSaved?.({ takerId: currentTakerId, testId, mainScore });
        } catch (sideErr) {
          console.warn('[onSaved] yan etki hatası (göz ardı edildi):', sideErr);
        }
      }
    } catch (err) {
  ...
  }, [result, studentId, testId, answers, saver, onSaved]);
