...
import {
  ...
  FileText, BookOpen, X, Send, Loader2
} from 'lucide-react';
    ...
    <div>
      ...
      {activeAssignments.length > 0 && (
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-300 rounded-2xl p-5 mb-4 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2 ml-13 pl-13" style={{ paddingLeft: '52px' }}>
            {activeAssignments.map((t) => (
              <span
                key={t}
                className="inline-flex items-center gap-1.5 bg-white border border-amber-300 text-amber-800 text-[12px] font-semibold px-3 py-1.5 rounded-full shadow-sm"
              >
                {TEST_LABELS[t] || t}
                <button
          ...
          {pending.length === 0 ? (
            ...
            <>
                ...
                {pending.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-[14px] font-semibold text-[#0f2847]">{TEST_LABELS[t] || t}</p>
                        {isAlreadyAssigned && (
              ...
              <div className="sticky bottom-4 z-10">
                ...
                >
                  {saving ? (
                    <><Loader2 className="w-4 h-4 animate-spin" /> Atanıyor...</>
                  ) : selected.size === 0 ? (
                    <>Test seçin</>
                  ) : (
          ...
          {completed.length === 0 ? (
            ...
            <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/40 shadow-sm overflow-hidden">
              {completed.map((c) => (
                <div
                  key={c.id}
                  className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-50 last:border-b-0"
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                    c.has_report ? 'bg-emerald-100' : 'bg-gray-100'
                  }`}>
                    <CheckCircle2 className={`w-4 h-4 ${c.has_report ? 'text-emerald-600' : 'text-gray-400'}`} />
                  </div>
                  <div className="flex-1">
                    <p className="text-[14px] font-semibold text-[#0f2847]">{TEST_LABELS[c.test_type] || c.test_type}</p>
                    <p className="text-[11px] text-gray-400">{formatDate(c.completed_at)}</p>
                  </div>
                  {c.has_report && (
                    <span className="text-[11px] text-emerald-700 font-bold bg-emerald-50 px-2 py-1 rounded-full flex items-center gap-1">
                      <FileText className="w-3 h-3" /> Rapor var
                    </span>
                  )}
                </div>
              ))}
