...
// DELETE /api/reports/holistic?id=XXX
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Öğrenciler kendi raporlarını silemez
    if (callerProfile.role === 'student') {
      return NextResponse.json({ error: 'Rapor silme yetkiniz yok.' }, { status: 403 });
    }

    // Raporu kontrol et, cross-school koruması
    ...
    console.error('[reports/holistic DELETE]', err);
    return NextResponse.json({ error: 'Sunucu hatası.' }, { status: 500 });
  }
}
