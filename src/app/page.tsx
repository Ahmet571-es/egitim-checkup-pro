...
import {
  ...
  CheckCircle2, Sparkles, Users, FileText, ChevronRight, Mail, Shield, Menu, X
} from 'lucide-react';
...
const FEATURES = ['10 bilimsel test', 'AI analiz raporu', "3'lü entegre rapor", 'PDF/Word export', '5 kullanıcı paneli'];

/* ═══ SUB-COMPONENTS ═══ */

/** Madde 2: Stat card with count-up */
...
export default function LandingPage() {
  ...
  // Madde 6: Navbar scroll detection
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#f0f5ff] via-[#f8fafc] to-[#f0fdf8]">
      ...
      <nav className={`sticky top-0 z-50 border-b transition-all duration-300 ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-2">
            ...
            <Link href="/register" className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-bold shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 hover:scale-[1.03] active:scale-[0.98] transition-all">
              Ücretsiz Başla
            </Link>
          </div>

          {/* Madde 26: Mobile hamburger with morph animation */}
          <button
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden absolute top-16 inset-x-0 bg-white/95 backdrop-blur-xl border-b border-gray-200/50 shadow-xl p-4 flex flex-col gap-2"
            ...
            <Link href="/register" onClick={() => setMobileMenuOpen(false)} className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-bold shadow-lg text-center">Ücretsiz Başla</Link>
          </div>
        )}
      </nav>

      {/* ═══ MADDE 1: HERO — CINEMATIC ENTRANCE ═══ */}
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-24 text-center">
          {/* Badge — drops in at 1000ms */}
          <div className="hero-badge inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/70 backdrop-blur-lg border border-white/50 shadow-sm mb-8">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-[#0f2847] leading-tight tracking-tight max-w-4xl mx-auto">
            ...
            <span className="hero-word-2 hero-word-3 inline-block bg-gradient-to-r from-emerald-500 to-teal-600 bg-clip-text text-transparent">
              Gerçekten Tanıyın
            </span>
          </h1>

          {/* Subtitle — slides in at 600ms */}
          <p className="hero-subtitle mt-6 text-lg sm:text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed">
            Psikometrik testler ve yapay zekâ ile her öğrencinin benzersiz potansiyelini keşfedin.
          </p>

          {/* CTA buttons — scale-in at 800ms */}
          <div className="hero-cta mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/register"
              className="touch-feedback px-8 py-4 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold shadow-xl shadow-emerald-500/25 hover:shadow-emerald-500/40 hover:scale-[1.03] active:scale-[0.98] transition-all flex items-center gap-2 text-[15px] pulse-glow">
              Ücretsiz Başla <ArrowRight className="w-5 h-5" />
            </Link>
            <a href="#testler"
              className="touch-feedback px-8 py-4 rounded-2xl bg-white/60 backdrop-blur-lg border border-white/50 text-[#0f2847] font-bold shadow-sm hover:shadow-md hover:bg-white/80 active:scale-[0.98] transition-all text-[15px] inline-flex items-center">
              Demo İncele
            </a>
          </div>
        </div>
      ...
      {/* ═══ MADDE 2: STATS — SCROLL-TRIGGERED COUNTERS ═══ */}
      ...
      {/* ═══ MADDE 3: TESTS — INTERACTIVE GALLERY ═══ */}
      <section id="testler" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 scroll-mt-20" ref={tests.ref}>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {TESTS.map((t, i) => (
            <div key={t.name}
              className={`group bg-white/70 backdrop-blur-xl rounded-2xl border border-white/40 border-l-[3px] ${t.border} p-5 shadow-sm
              ...
              <div className="flex items-start gap-4">
                <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${t.color} flex items-center justify-center shadow-lg shrink-0 group-hover:rotate-[5deg] group-hover:shadow-xl transition-all duration-300`}>
      ...
      {/* ═══ MADDE 4: HOW IT WORKS — STEP-BY-STEP REVEAL ═══ */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-24" ref={steps.ref}>
        ...
        <div className="grid sm:grid-cols-3 gap-6 relative">
          ...
          <div className="hidden sm:block absolute top-[56px] left-[16.7%] right-[16.7%] h-[2px] bg-gradient-to-r from-emerald-200 via-sky-200 to-violet-200 z-0" />
              ...
              <ul className="space-y-3 mb-8">
                {FEATURES.map((f, fi) => (
                  <li key={f} className="flex items-center gap-2.5 text-sm text-gray-600 group/feature">
                    <CheckCircle2
                      className="w-4 h-4 text-gray-300 group-hover/feature:text-emerald-500 transition-colors duration-200 shrink-0"
