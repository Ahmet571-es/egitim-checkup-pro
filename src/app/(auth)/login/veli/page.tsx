...
const STORAGE_KEY_EMAIL = 'ecup_parent_email';
const STORAGE_KEY_REMEMBER = 'ecup_parent_remember';
...
function ParentLoginInner() {
  ...
  useEffect(() => {
    ...
    const justRegistered = localStorage.getItem('ecup_parent_email');
    const savedRemember = localStorage.getItem(STORAGE_KEY_REMEMBER);

    if (justRegistered) {
      setEmail(justRegistered);
      localStorage.removeItem('ecup_parent_email');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Veli onay süreci: öğretmen tarafından henüz onaylanmamışsa giriş yapamaz.
      // Onay bilgisi user_metadata.is_approved'da.
      ...
      {pendingMsg && (
        <div className="mb-5 p-4 rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 flex items-start gap-2.5 text-sm text-amber-800">
          ...
          <div>
            ...
            <p className="text-[12.5px] text-amber-700">Çocuğunuzun öğretmenin onayı bekleniyor. Onaylandığında size bilgi verilecek.</p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
        ...
        <div>
          ...
          <div className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-md pointer-events-none">
            ...
            <input
              ...
              className="w-full pl-14 pr-3 py-3.5 rounded-xl border border-gray-200 bg-white text-sm font-medium focus:outline-none focus:ring-2 focus:ring-pink-500/30 focus:border-pink-400 transition-all"
              required
              autoComplete="email"
            />
          </div>
        ...
        <div>
          ...
          <div className="relative">
            <div className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-fuchsia-600 flex items-center justify-center shadow-md pointer-events-none">
            ...
            <input
              ...
              maxLength={72}
              className="w-full pl-14 pr-12 py-3.5 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/30 focus:border-pink-400 transition-all"
              required
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3.5 rounded-xl bg-gradient-to-r from-pink-500 via-rose-500 to-fuchsia-500 text-white text-[14px] font-extrabold shadow-lg shadow-pink-500/40 hover:shadow-xl hover:shadow-pink-500/50 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none active:scale-[0.98]"
        >
          {loading ? (
            <>
