...
// E-posta tetikleyicileri — opsiyonel, hata olsa bile kayıt akışını bozmaz
async function triggerWelcomeEmail(userId: string) {
  try {
    const { sendWelcomeEmail } = await import('@/lib/email/triggers');
    await sendWelcomeEmail(userId);
  } catch (e) {
    console.warn('[auth] Hoş geldin e-postası gönderilemedi:', e);
  }
}
...
export async function signUp(formData: FormData) {
  ...
  // Hoş geldin e-postası — kullanıcı ID'si user_metadata'dan alınır
  const { data: { user: newUser } } = await supabase.auth.getUser();
  if (newUser?.id) {
    triggerWelcomeEmail(newUser.id);
  }
